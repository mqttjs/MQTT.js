...
Connection.prototype.parse = function(buf) {
	...
	while (pos < len) {
		...
		if (len - pos < this.packet.length) {
			...
			/*
			var tmpBuffer = new Buffer( this.packet.length );
			this.buffer.copy(tmpBuffer, 0, pos, this.buffer.written);
			this.buffer = tmpBuffer;
			pos = 0;
			len = pos - len;
			//this.buffer.written = tmpBuffer.length;
			//this.buffer = new Buffer( len - pos );
			*/
