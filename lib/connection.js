...
Connection.prototype.parse = function(buf) {
  ...
  while (pos < len) {
    ...
    if (len - pos < this.packet.length) {
	  ...
	  var tmpBuffer = new Buffer( this.buffer.length - ( this.packet.length + pos ) );
	  this.buffer.copy( tmpBuffer, 0, this.packet.length + pos + 1 );
	  this.buffer = tmpBuffer;
      /*
      if ( this.packet.cmd == "connect" ) {
        if ( this.packet.keepalive ) {
          // add 10% to the time:
          var evictAfter = this.packet.keepalive;
          evictAfter += Math.floor(this.packet.keepalive * .1);
          this.stream.setTimeout(evictAfter*1000);
        }
      }
      */
      this.packet = {};
