...
parser.publish = function(buf, packet, encoding) {
  ...
  packet.payload = myPayloadParser(parser, buf);
...
function myPayloadParser(parser, buf) {	
	var result = '';
	for(var index = 0 ; index < parser._len ; index++){
		var val = buf[parser._pos + index];
		result += htos(val);
	}
...
function htos(hex){
	var secondChar = hex % 16;
	var firstChar = (hex - secondChar) / 16;	
	return hexToChar(firstChar) + hexToChar(secondChar);
}

function hexToChar(num){
	switch(num){
	case 0 : return '0';
	case 1 : return '1';
	case 2 : return '2';
