...
function MqttClient (streamBuilder, options) {
  ...
  // Ping timers, setup in _setupPingTimers
  this.sendPingTimer = null
  this.checkPingTimer = null
  ...
  // Setup ping timer
  this.on('connect', this._setupPingTimers)
  ...
  // Clear ping timer
  this.on('close', function () {
    if (that.sendPingTimer !== null) {
      that.sendPingTimer.clear()
      that.sendPingTimer = null
    }
    if (that.checkPingTimer !== null) {
      that.checkPingTimer.clear()
      that.checkPingTimer = null
    }
  })
...
MqttClient.prototype._cleanUp = function (forced, done) {
  ...
  if (this.sendPingTimer !== null) {
    this.sendPingTimer.clear()
    this.sendPingTimer = null
  }
  if (this.checkPingTimer !== null) {
    this.checkPingTimer.clear()
    this.checkPingTimer = null
  }
}
...
/**
 * _setupPingTimers - setup the ping timers (one for next send and one for the check)
 *
 * @api private
 */
MqttClient.prototype._setupPingTimers = function () {
  var that = this
  if (!this.options.keepalive) {
    return
  }

  if (!this.sendPingTimer) {
    this.pingResp = true
    this.sendPingTimer = reInterval(function () {
      that._sendPing()
    }, this.options.keepalive * 1000)
  }

  if (!this.checkPingTimer) {
    this.checkPingTimer = reInterval(function () {
      that._checkPing()
    }, this.options.keepalive * 1000)
  }
}
...
MqttClient.prototype._shiftPingInterval = function () {
  if (this.sendPingTimer && this.options.keepalive && this.options.reschedulePings) {
    this.sendPingTimer.reschedule(this.options.keepalive * 1000)
  }
}
/**
