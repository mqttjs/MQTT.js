...
class MqttClient extends EventEmitter {
	 ...
	 */
		...
		function closeStores() {
			...
			that.incomingStore.close((e1) => {
				that.outgoingStore.close((e2) => {
		...
		function startStreamProcess() {
			...
			function storeDeliver() {
				...
				if (!that.disconnecting && !that.reconnectTimer) {
					cb = that.outgoing[packet2.messageId]
						? that.outgoing[packet2.messageId].cb
						: null
					...
					if (that.messageIdProvider.register(packet2.messageId)) {
						that._sendPacket(packet2, undefined, undefined, true)
					} else {
						that.log(
