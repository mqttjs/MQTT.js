...
function MqttClient(streamBuilder, options) {
  ...
  this.on('connect', function() {
    ...
    function emitMaybe() {
      if (queue.length === 0 && inflights === 0) {
        return that.emit('queueEmpty');
      }
    }
    ...
    function deliver() {
      ...
      if (that.outgoing[packet.messageId]) {
        inflights++
        original = that.outgoing[packet.messageId]
...
MqttClient.prototype.end = function(cb) {
  ...
  if (!this.connected) {
    // wait a little if we do a sub-pub offline demo
    this.once('queueEmpty', finish);
  } else {
    this._cleanUp();
    closeStores();
  }
