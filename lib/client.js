  ...
  switch (qos) {
    ...
    case 0:
      ...
      this.handleMessage(packet, function (err) {
        if (qos > 0) {
          // for QOS 1, expect to send back the ack
          if (!err) {
            // if not error was returned, send the ack back
            that._sendPacket({
              cmd: 'puback',
              messageId: mid
            })
          } else {
            // error occured.  broker will resend message later
            // implmentations that have handleMessage respond with an error should have logic to decide whether or not to disconnect/reconnect
          }
        }
        done()
      })
