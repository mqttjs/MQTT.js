...
function sendPacket (client, packet, cb) {
  function emitAndCb () {
    if ('publish' === packet.cmd) {
      client.emit('publish', packet.topic, packet.payload, packet);
    }
    if (cb) {
      cb();
    }
  ...
  try {
    ...
    if (client.stream.write(buf) && (cb || 'publish' === packet.cmd)) {
      emitAndCb();
    } else if (cb || 'publish' === packet.cmd) {
      client.stream.once('drain', emitAndCb);
    }
  } catch (err) {
    if (cb) {
      cb(err);
