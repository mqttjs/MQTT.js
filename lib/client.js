...
var defaultConnectOptions = {
  ...
  baseReconnectPeriod: 1000,
  reconnectDeviance: 0.05,
  maxReconnectPeriod: 4000,
...
/**
 * _randomDevianceOnValue - generates a value with deviance
 * @param {number} value
 * @param {number} deviance
 */
function _devianceOnValue (value, deviance) {
  var min = value - (value * deviance)
  var max = value + (value * deviance)
...
/**
 * _backoff - this backoff algorithm uses fibonacci with
 *  a small deviance on each iteration
 * @param {number} iteration current iteration
 */
MqttClient.prototype._backOffReconnect = function (iteration) {
  var that = this
  var current = 1
  ...
  var delay = Math.min(that.options.maxReconnectPeriod, _devianceOnValue(current * that.options.baseReconnectPeriod, that.options.reconnectDeviance))

  that.reconnectTimer = setTimeout(function () {
    that._reconnect()
    that._backOffReconnect(iteration += 1)
  }, delay)
}
...
MqttClient.prototype._setupReconnect = function () {
  ...
  if (!that.disconnecting && !that.reconnectTimer) {
    ...
    if (that.options.backOffStrategy === 'jittered-fibonacci' && (that.options.baseReconnectPeriod > 0 && that.options.maxReconnectPeriod > 0)) {
      that._backOffReconnect(1)
    } else if (that.options.reconnectPeriod > 0) {
...
MqttClient.prototype._clearReconnect = function () {
  if (this.reconnectTimer) {
    if (this.options.backOffStrategy === 'jittered-fibonacci') {
