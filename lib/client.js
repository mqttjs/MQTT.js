...
function MqttClient(port, host, options) {
  ...
  this.net_client.connect(
      port, 
      host, 
      this.mqtt_conn.connect.bind(this.mqtt_conn, this.options)
  );
  ...
  this.mqtt_conn.on('unsuback', this._handleAck.bind(this));
