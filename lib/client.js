...
function MqttClient(streamBuilder, options) {
  ...
  // Inflight messages
  this.store = this.options.store || new Store();
  this.outgoing = {}
...
MqttClient.prototype._setupStream = function() {
  ...
  // MqttConnection
  this.conn = new Connection(this.stream, this.options);
...
function storeAndSend(client, packet, cb) {
  client.store.put(packet, function storedPacket(err) {
    if (err) {
      return cb && cb(err);
    }
    client.conn.write(packet, cb);
  })
}
...
MqttClient.prototype._handleAck = function(packet) {
  ...
  switch (type) {
    ...
    case 'suback':
      delete this.outgoing[mid];
      this.store.del(packet, function(err, original) {
        if (err) {
