...
MqttClient.prototype._cleanUp = function (forced, done) {
  ...
  if (forced) {
    ...
    debug('(%s)_cleanUp: destroying stream', that.options.clientId)
    this.stream.destroy()
  } else {
    var packet = xtend({ cmd: 'disconnect' }, opts)
    debug('(%s)_cleanUp: sending disconnect packet', that.options.clientId)
    this._sendPacket(
      packet,
      setImmediate.bind(
        null,
  ...
  if (done && !this.connected) {
    debug('(%s)_cleanUp: removing stream `done` callback `close` listener', that.options.clientId)
