...
MqttClient.prototype._handleAuth = function (packet) {
  ...
  if (rc === 24) {
    // hack: _sendPacket can only send packets if the client is connected - as we are in extended auth here
    // we are not connected yet - so we need to trick this -> should be changed in _sendPacket
    this.reconnecting = false
    this.connected = true
    this._sendPacket(packet)
    this.connected = false
 ...
 * @api public
 */
MqttClient.prototype.handleAuth = function (packet) { }
