...
MqttClient.prototype._handlePublish = function(packet) {
  var topic = packet.topic
    ...
    , puback_called = false
    , puback = (function () {
        puback_called || this.conn.puback({messageId: mid});
        puback_called = true;
    }).bind(this);

  switch (qos) {
    case 0:
      this.emit('message', topic, message, packet, function () {});
      break;
    case 1:
      if (!this.emit('message', topic, message, packet, puback)) {
        puback();
      }
      break;
    case 2:
...
MqttClient.prototype._handlePubrel = function(packet) {
  ...
  if (pub) {
    if (!this.emit('message', pub.topic, pub.payload, pub, pubcomp)) {
      pubcomp();
    }
  }
  else {
