...
module.exports.connect = function(opts) {
  var opts = opts || {}
    ...
    , protocolVersion = opts.protocolVersion || defaultProtocolVer
    , will = opts.will
    , clean = opts.clean
    , keepalive = opts.keepalive || defaultKeepalive
    , clientId = opts.clientId || defaultClient()
  ...
  /* Protocol ID must be a string but it can be empty */
  if(typeof protocolId !== "string") return null;
  /* Protocol version number must be a one byte number */
  if(typeof protocolVersion !== "number" || protocolVersion < 0 || protocolVersion > 255) return null;
