...
module.exports.createSecureClient = function(port, host, opts) {
  ...
  if (opts.keyPath && opts.certPath) {
    ...
    tls_opts.rejectUnauthorized = opts.rejectUnauthorized;
  }

  tls_client = tls.connect(port, host, tls_opts, function() {
    console.log("tls client connected: ", tls_client.authorized ? 'authorized' : 'unauthorized')
  });
  ...
  // Samir - adding these lines from node tls documentation
  // http://nodejs.org/api/tls.html#tls_tls_connect_port_host_options_callback
  tls_client.setEncoding('utf8');
    tls_client.on('data', function(data) {
    console.log("tls_client got data");
    console.log(data);
  });
  tls_client.on('end', function() {
    console.log("tls_client got end event");
    //server.close();
  });
  tls_client.on('connect', function() {
    console.log("tls_client on connect");
  });
