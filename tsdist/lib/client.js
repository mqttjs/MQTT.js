...
var MqttClient = (function (_super) {
    ...
    function MqttClient(streamBuilder, options) {
        ...
        _this.pingResp = false;
        // MessageIDs starting with 1
        _this.nextId = Math.floor(Math.random() * 65535);
        // Packet queue
        _this.queue = [];
        ...
        for (var k in defaultConnectOptions) {
            if (typeof _this.options[k] === 'undefined') {
                _this.options[k] = defaultConnectOptions[k];
            }
            else {
        ...
        // Setup ping timer
        _this.on('connect', _this._setupPingTimer);
        // Send queued packets
        _this.on('connect', function () {
        ...
        _this.on('close', _this._setupReconnect);
        _this._setupStream();
        return _this;
    }
    MqttClient.prototype.on = function (event, cb) {
        _super.prototype.on.call(this, event, cb);
        return this;
    };
    MqttClient.prototype.once = function (event, cb) {
        _super.prototype.once.call(this, event, cb);
        return this;
    };
    /**
    ...
    MqttClient.prototype._setupStream = function () {
        var connectPacket;
        var that = this;
        var writable = new readable_stream_1.Writable();
    ...
    MqttClient.prototype.subscribe = function () {
        ...
        this.outgoing[packet.messageId] = function (err, subAck) {
            if (!err) {
                subs.forEach(function (sub) {
                    that._subscribedTopics[sub.topic] = sub.qos;
                });
                var granted = subAck.granted;
     ...
     * @api private
     */
    MqttClient.prototype._handlePingresp = function (packet) {
        this.pingResp = true;
    };
    /**
     * _handleConnack
